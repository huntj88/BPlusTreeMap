...
sealed class LeafNode<Key : Comparable<Key>, Value> {
    object EmptyNode: LeafNode<Nothing, Nothing>()

    class LeafNodeImpl<Key : Comparable<Key>, Value> : LeafNode<Key, Value>(), Node<Key, Value> {
        private val entries: Array<Entry<Key, Value>?> = Array(numEntriesPerNode) { null }
        private var leftLink: LeafNode<Key, Value> = EmptyNode as LeafNode<Key, Value>
        private var rightLink: LeafNode<Key, Value> = EmptyNode as LeafNode<Key, Value>
    ...
    override fun getRange(start: Key, endInclusive: Key): List<Entry<Key, Value>> {
        ...
        private fun splitLeaf(newEntry: Entry<Key, Value>): PutResponse.NodeFull<Key, Value> {
            ...
            val left = LeafNodeImpl<Key, Value>().also { node ->
            ...
            val right = LeafNodeImpl<Key, Value>().also { node ->
            ...
            (leftLink as? LeafNodeImpl)?.let {
                it.rightLink = left
            }
            (rightLink as? LeafNodeImpl)?.let {
                it.leftLink = right
            }
